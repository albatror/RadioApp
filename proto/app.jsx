...
// ---------- Visualizer ----------
function Visualizer({ playing, paused }) {
  const BARS = 64;
  ...
  useEffect(() => {
    if (!playing) return;
    let raf;
    const tick = () => { force(t => t + 1); raf = requestAnimationFrame(tick); };
  ...
  return (
    <div className={"visualizer" + (paused ? " paused" : "")}>
      {phases.map((ph, i) => {
        // Pseudo-FFT: bell curve weight + multi-sine
        const x = (i / BARS) * 2 - 1;
...
// ---------- Sidebar ----------
function Sidebar({ active, setActive, listeners, lang }) {
  const t = (en, fr) => lang === "fr" ? fr : en;
  const items = [
...
function App() {
  ...
  // Wobble listener count for life
  useEffect(() => {
    const id = setInterval(() => setListeners(l => Math.max(180, Math.min(380, l + Math.round((Math.random() - .5) * 6)))), 4000);
    return () => clearInterval(id);
  }, []);
  ...
  return (
    <div className={"app " + (paletteCls === "palette-terracotta" ? "" : paletteCls)}>
      <div className="shell">
        <Sidebar active={active} setActive={setActive} listeners={listeners} lang={lang} />
        <main className="main">
  ...
  return (
    <TweaksPanel title="Tweaks">
      ...
      <TweakSection label="Modules">
        <TweakToggle label="Show schedule grid" value={tweaks.showSchedule} onChange={(v) => setTweak("showSchedule", v)} />
        <TweakToggle label="Show genre tiles"   value={tweaks.showGenres}   onChange={(v) => setTweak("showGenres", v)} />
